    ...
    [self fetchQuantitySamplesOfType:bodyFatPercentType
                          ...
                          completion:^(NSArray *results, NSError *error) {
                              if(results){
                                  ...
                                  NSLog(@"error getting body fat percent samples: %@", error);
                                  callback(@[RCTMakeError(@"error getting body fat percent samples", error, nil)]);
                                  return;
                              }
                          }];
}
    ...
    [self fetchQuantitySamplesOfType:waistCircumferenceType
                          ...
                          completion:^(NSArray *results, NSError *error) {
                              if(results){
                                  ...
                                  NSLog(@"error getting waist circumference samples: %@", error);
                                  callback(@[RCTMakeError(@"error getting waist circumference samples", error, nil)]);
                                  return;
                              }
                          }];
}
